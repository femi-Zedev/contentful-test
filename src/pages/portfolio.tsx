...
export default function Portfolio() {
    ...
    return (
        <div className="container  mb-8 px-4 sm:px-5 md:px-10 lg:px-[60px]">
            <div className="py-12">
                <h2 className="after-effect after:left-52">{portfolioContent.title}</h2>
            ...
            <div className="">
                <div className="grid-sizer"></div>
                {
                    filteredWorks.length > 0 ?
                        <MasonryLayout columns={viewPortWidth == 'sm' || viewPortWidth == 'xs' ? 1 : 2} gap={24}>
                            {filteredWorks.map((item: any, i: number) => (
                                <PortfolioItem key={i} colorScheme={colorScheme} item={item} animate={isAnimating} viewPortWidth={viewPortWidth} />
                            ))}
                        </MasonryLayout>
                        :
...
export function PortfolioItem({ colorScheme, item, animate, viewPortWidth }: { colorScheme: string, item: PortfolioItemType, animate: boolean, viewPortWidth: string }) {
    const { title, imgSrc, category } = item
    const [opened, { open, close }] = useDisclosure(false);

    return (
        <>
            <Modal centered radius="lg" size={viewPortWidth == 'sm' || viewPortWidth == 'xs' ? 'lg' : '60%'}  opened={opened} onClose={close}
                styles={{ 
                    content: { background: colorScheme == 'dark' ? "#ffffff" : '#191919'},
                    header: { background: colorScheme == 'dark' ? "#ffffff" : '#191919'},
                 }}
                title={
                    <h2 className="w-full text-gray-500 text-4xl text-center font-bold">
                        {title}
                    </h2>
                }
            >
                <div className="overflow-y-scroll h-auto md:h-[64vh] px-2 mb-5">
                    <img className="w-full h-auto object-cover rounded-xl " src={imgSrc} alt="portfolio image" />
                </div>
            </Modal>
